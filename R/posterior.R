...
getPosteriorI <- function(
  ...
  # prod(post_list$Ctr[1,comp_mat_ind[x,1]], post_list$DG_1[1,comp_mat_ind[x,2]], post_list$DG_2[1,comp_mat_ind[x,3]], post_list$DG_3[1,comp_mat_ind[x,4]], post_list$DG_4[1,comp_mat_ind[x,5]])

  post_weights <- lapply(1:nrow(comp_mat_ind), compute_product,
                         mat = comp_mat_ind, post_list = post_list)
  ...
  attr(post_list, "covariance matrices") <- lapply(seq_along(diagonals), function(x) diag(diagonals[[x]])^2)
  attr(post_list, "Post weights") <- post_weights
...
postMix2posteriorList <- function (
  ...
  attr(posterior_list_RBesT, "Post weights")<-posterior_list$weights
...
is_off_diagonal_zero <- function(mat) {
  all(mat[row(mat) != col(mat)] == 0)
}

compute_product <- function(mat, row_index, post_list) {
  indices <- mat[row_index, ]
  post_list_names <- names(post_list)
  values <- mapply(function(name, col_index) {
    post_list[[name]][1, col_index]
  }, post_list_names, indices)
  prod(values)
}
