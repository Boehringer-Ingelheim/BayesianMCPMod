...
[BayesianMCPMod](https://CRAN.R-project.org/package=BayesianMCPMod)
package for sample size calculations and the comparison with the
[MCPModPack](https://CRAN.R-project.org/package=MCPModPack) package.
Bayesian MCPMod is set up in a way that it mimics the results (and
operating characteristics) of the frequentist MCPMod for vague priors.
This characteristic is illustrated in the following sections focusing on
the trial planning.

The following dose-finding scenario is considered to compare Bayesian
MCPMod and MCPModPack success probabilities:

- Four dose levels plus placebo (0 mg, 1 mg, 2 mg, 4 mg, 8 mg)

- Total sample size of N = 200 with an equal allocation ratio for each
...
Simulations are performed with 10000 runs. Given the number of
...
![](Simulation_Comparison_files/figure-html/unnamed-chunk-6-1.png)

## Varying the Expected Effect for Maximum Dose

The following expected effects are studied:

- Expected effect for maximum dose of 0.0001, 0.05, 0.1, 0.2, 0.3, and
  0.5
...
The following plot shows the differences between the results obtained
...
![](Simulation_Comparison_files/figure-html/unnamed-chunk-12-1.png)

As expected, the operating characteristics of BayesianMCPMod with vague
priors match the operating characteristics of frequentist MCPMod.
...
In the following simulations, we examine the convergence of power and
...
The R package MCPModPack provides a final power result for a given
...
This is why the simulation is repeated for different values of number of
...
In the BayesianMCPMod implementation, only one simulation run is
...
The figure below shows the convergence of the power and success rate
...
![](Simulation_Comparison_files/figure-html/unnamed-chunk-22-1.png)
