...
kable(results_min_MCP)%>%
  kable_classic(full_width = TRUE)%>%
    ...
    add_header_above(c("Minimal scenario " = 4), font_size = 15, bold = TRUE)
...
list_models <- list(sim_models_monotonic_linear, sim_models_monotonic_exp, sim_models_monotonic_logistic, sim_models_monotonic_sigemax, sim_models_monotonic_emax)
chunks <- chunkVector(seq_along(list_models), getDoParWorkers())
...
results_monotonic_MCP <- data.table(
  ...
  Emax = c(results_list_monotonic[[2]]$sim_results$power),
  Logistic = c(results_list_monotonic[[3]]$sim_results$power),
  sigEmax = c(results_list_monotonic[[4]]$sim_results$power)
