---
title: "Simulation Example Comparing MCPMod and Bayesian MCPMod"
format: 
  html:
    fig-height: 3.5
    self-contained: true
    ...
    warning: false
vignette: >
  %\VignetteIndexEntry{Simulation Example Comparing MCPMod and Bayesian MCPMod}
...
To compare Bayesian MCPMod and MCPModPack success probabilities, four different dose-finding scenarios are considered.
For the first three scenarios, the general trial design is the same except the considered set of dose response models. 
...
The fourth scenario, the variability scenario, has  different design choices for the number of dose levels and the total sample size: 
...
In a subsequent step, the analysis is extended to test the convergence of power values as the number of simulations increases.
For this part of the study, an expected effect for the maximum dose of 0.2 is assumed.
The sample size is set to 200 for the linear, monotonic, and non-monotonic scenario and 100 for the variability scenario using equal allocation. 
...
nsample_list <- list(20 * c(2, 1, 1, 1, 2),
                     30 * c(1, 1, 1, 1, 1),
                     24 * c(2, 1, 1, 1, 2),
                     36 * c(1, 1, 1, 1, 1))
nsample_vector <- c("(40,20,20,20,40)",
                    "(30,30,30,30,30)",
                    "(48,24,24,24,48)",
                    "(36,36,36,36,36)")
...
Nsample <- c(40, 40, 40, 40, 40)
expectedEffect_fix <- 0.2
...
nsample_list_var <- list(20 * c(2, 1, 1, 2),
                         30 * c(1, 1, 1, 1),
                         24 * c(2, 1, 1, 2),
                         36 * c(1, 1, 1, 1))

# define model functions  
emax.g    <- guesst(d = doses.sim[2], p = 0.6, model = "emax") 
exp.g     <- guesst(d = doses.sim[2], p = 0.05, model = "exponential", Maxd = max.dose)
logit.g   <- guesst(d = c(doses.sim[2], doses.sim[3]), p = c(0.1, 0.9), model = "logistic", Maxd = max.dose) 
...
beta.g    <- guesst(d = doses.sim[2], p = 0.2, model = "betaMod",
                    dMax = 5.5, scal = 9.6, Maxd = max.dose)
...
results_min_MCP <- foreach(k = chunks, .combine = c)  %dorng% {
  
  lapply(k, function (i) {
    
    MCPModSimulation(endpoint_type = "Normal",
                     models = min_modelsPack,
                     alpha = alpha,
                     direction = "increasing",
                     model_selection = "aveAIC",
                     Delta = 0.1,
                     sim_models = list_models[[i]],
...
```{r warning=FALSE}
#| include: false
if (file.exists("min_scenario_var_expected_effect.rds")) {
  
  results_list_min <- readRDS("min_scenario_var_expected_effect.rds")
  ...
  saveRDS(results_list_min, "min_scenario_var_expected_effect.rds")
...
results_min_MCP <- data.table(
  ...
  mutate(Average = rowMeans(select(., linear:emax)))

kable(results_min_MCP) %>%
  kable_classic(full_width = TRUE) %>%
    add_header_above(c("Power results of different expected effects " = 5), font_size = 15, bold = TRUE) %>%
...
list_models <- c(rep(list(sim_models_min_linear), 4),
                 rep(list(sim_models_min_exp), 4),
                 rep(list(sim_models_min_emax), 4))

# nsample_list = rep(nsample_list, length.out = length(list_models))
...
results_list_min_nsample <- foreach(k = chunks, .combine = c)  %dorng% {
  
  lapply(k, function (i) {
    
    sim_parameters = list(n = rep(nsample_list, length.out = length(list_models))[[i]],
                          doses = doses.sim,
                          dropout_rate = 0.0,
                          go_threshold = 0.1,
                          nsims = n_sim)
...
```{r warning=FALSE}
#| include: false
if (file.exists("min_scenario_var_sample_size.rds")) {
  
  results_list_min_nsample <- readRDS("min_scenario_var_sample_size.rds")
  ...
  saveRDS(results_list_min_nsample, "min_scenario_var_sample_size.rds")
...
results_min_MCP_nsample <- data.table(
  N_sample = c("(40,20,20,20,40)",
               "(30,30,30,30,30)",
               "(48,24,24,24,48)",
               "(36,36,36,36,36)"),
  linear = c(results_list_min_nsample[[1]]$sim_results$power,
             results_list_min_nsample[[2]]$sim_results$power,
             results_list_min_nsample[[3]]$sim_results$power,
             results_list_min_nsample[[4]]$sim_results$power),
  exponential = c(results_list_min_nsample[[5]]$sim_results$power,
                  results_list_min_nsample[[6]]$sim_results$power,
                  results_list_min_nsample[[7]]$sim_results$power,
                  results_list_min_nsample[[8]]$sim_results$power),
  emax = c(results_list_min_nsample[[9]]$sim_results$power,
           results_list_min_nsample[[10]]$sim_results$power,
           results_list_min_nsample[[11]]$sim_results$power,
           results_list_min_nsample[[12]]$sim_results$power)) %>%
  mutate(Average = rowMeans(select(., linear:emax)))
