...
load_path <- "/fsx/home/wojcieko/GitHub/BayesianMCPMod/inst/extdata"
...
In the following plot shows the considered candidate models.
...
The results of BayesianMCPMod are shown as a horizontal line and the difference to the result of MCPModPack is presented as a vertical line.
As the results are so close to one another, the vertical lines are barely visible.
The results of the different assumed true dose-response models, which are the basis for simulating the data, are shown in different colours.
...
Numerical results are shown in the following two tables.
...
For this, the same simulation settings apply as in @sec-introduction, only the expected maximum effect is fixed at 0.2. 
...
The R package MCPModPack provides a final power result for a certain number of simulations and does not allow accessing intermediate power values.

```{r}
#| code-summary: Number of simulations for power values
n_sim_vec <- c(10, 125, 250, 500, 1000, 2000)
```
...
This is why the simulation is repeated for different values of number of simulations.
...
The following plot shows the convergence of the power and success rate values for an increasing number of simulations.
...
ggplot(df_plot_conv, aes(x = n_sim, y = success_rate_diff, color = model_name)) +
  ...
  scale_x_continuous(breaks = unique(df_plot_conv$n_sim)) + 
  labs(
